...
- `.config/k8s/main/db.yml` and `.config/k8s/next/db.yml` define the MySQL Deployments (`db` and `db-next`), their PVC mounts, and explicit MySQL memory-related arguments.
...
- Aircraft coordinates and historical turns have independent update paths. `setVatsimMandatoryData()` mutates keyed pilot coordinates and supplies smoothing samples, while visible smoothed aircraft are advanced by RAF; off-screen/load-fallback aircraft receive direct geometry writes that also reseed interpolation. Historical turn features are fetched separately from the QuestDB-backed `/api/data/vatsim/pilot/:cid/turns` endpoint, so a growing track does not prove that the aircraft geometry path is still advancing.
...
- `app/components/map/layers/MapAircraftList.vue` owns aircraft preparation (`setVisiblePilots`), the `showTracks` output, and render watchers. Mandatory/short updates and extent/settings changes initiate preparation; `showTracks` is consumed by the render watcher rather than fed back into preparation.
- Aircraft position has separate representations: the OpenLayers Point geometry, the feature's `coordinates`/`heading` properties (used by `MapPopupAircraft.vue`), and interpolation samples/accumulators in `smooth.ts`. Check all three when reviewing direct-position updates or transitions into/out of the rendered set.
- `app/composables/render/aircraft/state.ts` owns lightweight CID-keyed track/flight-plan state, abort controllers and per-attempt request deadlines. Raw turns responses stay local to the fetch/build pass. `MapAircraftList.vue` owns separate dynamic-line and static-history sources and disposes track state with those layers. `app/composables/render/aircraft/lines.ts` synchronizes live endpoints for feed and RAF updates.
- `NavigraphRoute.vue` owns a dedicated live-connector source/layer, registered with smoothing; static route features remain in the shared Navigraph source. `app/composables/render/airports/layers/layout.ts` exposes `disposeAirportLayouts()` for its retained layout features, called when airport layers hide or unmount.
- `app/composables/vatsim/airport.ts` owns shared short-feed CID/order indexes for airport aircraft panels. `AirportAircraft.vue` uses `app/composables/virtual-chunks.ts` for measured variable-height/wrapped row chunks and maintains CID-indexed pilot statistics.
...
- `node scripts/tests/client-performance.test.mjs` runs offline regression scenarios against transpiled production functions with explicit Nuxt/browser dependencies: coordinate/smoothing transitions, track retries/lifecycle/history isolation, antimeridian geometry, airport candidates/layout cleanup, and virtual chunks. It does not replace `yarn typecheck`, `yarn lint`, or browser profiling.
