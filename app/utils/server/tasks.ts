        ...
        if (status.activeIncidents?.filter(x => x.status !== 'RESOLVED').length) {
            ...
            if (current.status === 'INVESTIGATING' || current.status === 'IDENTIFIED') {
                ...
                notam = {
                    ...
                    text: `The VATSIM incident is currently being monitored. The team is continuing to assess service stability. Visit the <a href="${ current.url }" class="__link" target="_blank">status page</a> for more information.`,
                    active: true,
                    activeFrom: null,
                    activeTo: null,
                    dismissable: true,
