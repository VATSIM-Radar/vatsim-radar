...
export const processNavdataHoldings: NavdataProcessFunction = async ({ fullData, shortData, db }) => {
    ...
    for (const item of holdings) {
        ...
        const waypoint = shortData.waypoints?.[`${ item.waypoint_identifier }-${ item.icao_code }-${ item.area_code }-${ item.region_code ?? 'default' }`];
...
export const processNavdataWaypoints: NavdataProcessFunction = async ({ fullData, shortData, db }) => {
    ...
    for (const item of waypoints) {
        ...
        shortData.waypoints[`${ item.waypoint_identifier }-${ item.icao_code }-${ item.area_code }-${ item.region_code ?? 'default' }`] = [item.waypoint_identifier, item.waypoint_longitude, item.waypoint_latitude, item.waypoint_type, !!item.is_terminal];
    }
};
...
export const processNavdataAirways: NavdataProcessFunction = async ({ fullData, shortData, db }) => {
    ...
    for (let i = 0; i < airways.length; i++) {
        ...
        if (previousAirway && (previousAirway.seqno + 5 === airway.seqno || previousAirway.seqno + 10 === airway.seqno) && airway.route_identifier === previousAirway.route_identifier) continue;
        ...
        while (nextItem && airway.route_identifier === nextItem.route_identifier && (previousSeqno + 10 === nextItem.seqno || previousSeqno + 5 === nextItem.seqno)) {
        ...
        for (const airway of nextAirways) {
            ...
            const waypoint = shortData.waypoints?.[`${ airway.waypoint_identifier }-${ airway.icao_code }-${ airway.icao_code }-${ airway.area_code }-default`];
